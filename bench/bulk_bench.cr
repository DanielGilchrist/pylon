...
puts "tree:      #{files} files, #{(total_bytes / (1024.0 * 1024.0)).round(1)} MiB (generated in " \
     "#{(Time.instant - generated).total_seconds.round(1)}s)"
...
puts "bulk push: #{elapsed.total_seconds.round(2)}s " \
     "(#{(total_bytes / (1024.0 * 1024.0) / elapsed.total_seconds).round(1)} MiB/s)"
puts "outcomes:  #{applied} applied, #{skipped} skipped, #{report.conflicts.size} conflicts, " \
