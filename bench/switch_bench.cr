...
local_root = ARGV[0]? || abort(
  "usage: switch_bench <local_root> <remote_root> <switch_command> [binary]",
)
...
opened =
  if (latency = ENV["BULK_LATENCY_MS"]?) || ENV["BULK_RATE_BYTES"]?
    ...
    arguments = [latency || "0", rate, binary] + remote_arguments
...
puts "verify:    converged=#{verify.quiet?} (no-op cycle " \
     "#{(Time.instant - started).total_seconds.round(2)}s)"
