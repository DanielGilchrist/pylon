...
```sh
crystal tool unreachable src/pylon.cr spec/**/*_spec.cr
```

It lists methods nothing reaches from those entrypoints. It always exits 0, so read the output: `lib` bindings and platform-specific watchers show up as noise on the wrong platform, but a method you touched appearing in the list means you left dead code behind and should delete it.
