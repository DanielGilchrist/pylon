module Pylon::Compress
  class Dictionary
    ...
    def self.decompress(frame : Bytes, prefix : Bytes, limit : Int32) : Bytes | Problem
    ...
    def compress(source : Bytes, prefix : Bytes, into : Bytes) : Bytes | Problem
    ...
    def decompress(frame : Bytes, prefix : Bytes, limit : Int32) : Bytes | Problem
      size = declared_size(frame)
      if size == CONTENT_SIZE_UNKNOWN
        return Problem.new("the frame does not declare its content size")
      end
      return Problem.new("the frame header is invalid") if size == CONTENT_SIZE_ERROR
      if size > limit
        return Problem.new("the frame claims #{size} bytes, over the #{limit} byte limit")
      end
    ...
    private def configure_compressor(window_log : Int32, prefix : Bytes) : Problem?
    ...
    private def configure_decompressor(prefix : Bytes) : Problem?
    ...
    private def tune(parameter : LibZstd::CParameter, value : Int32) : Problem?
      Compress.check(LibZstd.cctx_set_parameter(@compressor, parameter, value))
    end

    private def reference_for_compression(prefix : Bytes) : Problem?
      Compress.check(
    ...
    private def reference_for_decompression(prefix : Bytes) : Problem?
      Compress.check(
    ...
    private def write_frame(source : Bytes, into : Bytes) : Bytes | Problem
    ...
    private def read_frame(frame : Bytes, into : Bytes) : Bytes | Problem
