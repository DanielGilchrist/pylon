...
describe Pylon::Write::Guard do
  ...
  it "refuses to act when the expected file vanished" do
    Guard.check(Entry.file(DIGEST), cached, nil).should eq(Verdict::ModificationDetected)
  end

  it "permits replacing a file that still matches the cache and the expected digest" do
    Guard.check(Entry.file(DIGEST), cached, observed).should eq(Verdict::Proceed)
  end

  it "refuses without cache evidence, rather than assuming safety" do
    Guard.check(Entry.file(DIGEST), nil, observed).should eq(Verdict::UnknownState)
  end

  it "refuses when the file changed since it was scanned" do
    Guard.check(Entry.file(DIGEST), cached, observed(size: 101_u64)).should eq(Verdict::ModificationDetected)
    Guard.check(Entry.file(DIGEST), cached, observed(mtime_ns: MTIME + 1)).should eq(Verdict::ModificationDetected)
    Guard.check(Entry.file(DIGEST), cached, observed(inode: 8_u64)).should eq(Verdict::ModificationDetected)
  end

  it "refuses when only the permissions changed" do
    executable = observed(mode: (LibC::S_IFREG | 0o755).to_u32)

    Guard.check(Entry.file(DIGEST), cached, executable).should eq(Verdict::ModificationDetected)
  end

  it "refuses when the cached digest disagrees with what we planned against" do
    Guard.check(Entry.file(OTHER), cached, observed).should eq(Verdict::ModificationDetected)
  end

  it "refuses when a file was replaced by a directory" do
    directory = observed(mode: (LibC::S_IFDIR | 0o755).to_u32)

    Guard.check(Entry.file(DIGEST), cached, directory).should eq(Verdict::ModificationDetected)
  end
  ...
  it "checks kind alone for directories and symlinks" do
    ...
    Guard.check(Entry.directory, nil, directory).should eq(Verdict::Proceed)
    Guard.check(Entry.directory, nil, link).should eq(Verdict::ModificationDetected)
    Guard.check(Entry.symlink("t"), nil, link).should eq(Verdict::Proceed)
    Guard.check(Entry.symlink("t"), nil, directory).should eq(Verdict::ModificationDetected)
  end

  it "never proceeds against unsynchronizable expectations" do
    Guard.check(Entry.untracked, nil, observed).should eq(Verdict::UnknownState)
    Guard.check(Entry.problematic("x"), nil, observed).should eq(Verdict::UnknownState)
  end
end
